...
### Méthodos de validación

| Método | Cómo funciona | Cuándo usar |
|--------|--------------|-------------|
| **HTTP-01** | Let's Encrypt visita `http://tudominio.com/.well-known/acme-challenge/<token>` y verifica que responde con el valor esperado | Servidor web accesible por HTTP (puerto 80) |
| **DNS-01** | Creas un registro TXT `_acme-challenge.tudominio.com` con el valor esperado | Sin servidor web, o detrás de un proxy/CDN |
...
> [!quote] La clave
> No hay excusa para no usar HTTPS en 2024. Let's Encrypt + Certbot o Caddy hacen que sea automático, gratuito y sin fricción.
...
| DNS y Cloudflare | [[04-dns-cdn-y-cdnflare]] (DNS-01 challenge, SSL modes) |
| DNS registros | [[03-dns-profundo]] (CAA, TXT para DNS-01) |
| Nginx | [[14-servidores-procesos-processos]] (configuración del servidor) |
